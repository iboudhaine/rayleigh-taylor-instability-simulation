...
static int parse_args(int argc, char **argv, Config *c) {
  ...
  while ((opt = getopt_long(argc, argv, "h", opts, &idx)) != -1) {
    ...
    case O_G:
      ...
      break;
    case O_RHOH:
      c->rho_heavy = atof(optarg);
      break;
    case O_RHOL:
      c->rho_light = atof(optarg);
      break;
    case O_P0:
      c->p0 = atof(optarg);
      break;
    case O_PERT:
      c->perturbation_amp = atof(optarg);
      break;
    case O_SEED:
      c->seed = atoi(optarg);
      break;
    case O_SCHEME:
      ...
      strncpy(c->scheme, optarg, sizeof(c->scheme) - 1);
      break;
    case O_CFL:
      c->cfl = atof(optarg);
      break;
    case O_K1:
      c->k1_coef = atof(optarg);
      break;
    case O_K2:
      c->k2_coef = atof(optarg);
      break;
    case O_K3:
      c->k3_coef = atof(optarg);
      break;
    case O_STEPS:
      c->steps = atol(optarg);
      break;
    case O_TEND:
      ...
      break;
    case O_SAVE:
      c->save_interval = atoi(optarg);
      break;
    case O_OUT:
      strncpy(c->output_dir, optarg, sizeof(c->output_dir) - 1);
      break;
    case O_NOOUT:
      c->no_output = 1;
      break;
    case O_OVER:
      ...
      break;
    case O_WARM:
      c->warmup_steps = atoi(optarg);
      break;
    case O_REP:
      c->repeats = atoi(optarg);
      break;
    case 'h':
      usage(argv[0]);
      exit(0);
...
static void state_init(State *s, int Nx, int Ny) {
  ...
  double **slots[] = {
      ...
      &s->u,       &s->v,       &s->ruu,      &s->ruv,      &s->rvv,
      &s->eu_p,    &s->ev_p,    &s->dr_dx,    &s->dr_dy,    &s->d2r_dx2,
      &s->d2r_dy2, &s->dru_dx,  &s->dru_dy,   &s->d2ru_dx2, &s->d2ru_dy2,
      &s->drv_dx,  &s->drv_dy,  &s->d2rv_dx2, &s->d2rv_dy2, &s->de_dx,
      &s->de_dy,   &s->d2e_dx2, &s->d2e_dy2,  &s->dp_dx,    &s->dp_dy,
      &s->dummy1,  &s->dummy2,  &s->druu_dx,  &s->druv_dx,  &s->druv_dy,
      &s->drvv_dy, &s->deup_dx, &s->devp_dy,  &s->rhs_r,    &s->rhs_ru,
      &s->rhs_rv,  &s->rhs_e,   &s->r_s,      &s->ru_s,     &s->rv_s,
      &s->e_s,     &s->p_s,     &s->rhs_r_s,  &s->rhs_ru_s, &s->rhs_rv_s,
      &s->rhs_e_s};
...
static void initialize_grid(const Config *c, State *s) {
  ...
  for (int i = 0; i < Nx; i++) {
    for (int j = 0; j < Ny; j++) {
      ...
      s->e[IX(i, j)] =
          s->p[IX(i, j)] / (c->gamma - 1) + 0.5 * rho * (u * u + v * v);
    }
  }
}
...
static void compute_derivatives(const double *f, double *dfx, double *dfy,
                                double *d2x, double *d2y, int Nx, int Ny,
                                double dx, double dy) {
  for (int i = 1; i < Nx - 1; i++) {
    for (int j = 1; j < Ny - 1; j++) {
      ...
      d2x[IX(i, j)] =
          (f[IX(i + 1, j)] - 2 * f[IX(i, j)] + f[IX(i - 1, j)]) / (dx * dx);
      d2y[IX(i, j)] =
          (f[IX(i, j + 1)] - 2 * f[IX(i, j)] + f[IX(i, j - 1)]) / (dy * dy);
    }
  }
}

static void compute_rhs(const Config *c, State *s, const double *r,
                        const double *ru, const double *rv, const double *e,
                        const double *p, double k1, double k2, double k3,
                        double *rhs_r, double *rhs_ru, double *rhs_rv,
                        double *rhs_e) {
  int Nx = c->Nx, Ny = c->Ny;
  double dx = c->Lx / Nx, dy = c->Ly / Ny;
  size_t n = (size_t)Nx * Ny;
  ...
  compute_derivatives(r, s->dr_dx, s->dr_dy, s->d2r_dx2, s->d2r_dy2, Nx, Ny, dx,
                      dy);
  compute_derivatives(ru, s->dru_dx, s->dru_dy, s->d2ru_dx2, s->d2ru_dy2, Nx,
                      Ny, dx, dy);
  compute_derivatives(rv, s->drv_dx, s->drv_dy, s->d2rv_dx2, s->d2rv_dy2, Nx,
                      Ny, dx, dy);
  compute_derivatives(e, s->de_dx, s->de_dy, s->d2e_dx2, s->d2e_dy2, Nx, Ny, dx,
                      dy);
  compute_derivatives(p, s->dp_dx, s->dp_dy, s->dummy1, s->dummy2, Nx, Ny, dx,
                      dy);
  compute_derivatives(s->ruu, s->druu_dx, s->dummy1, s->dummy2, s->dummy2, Nx,
                      Ny, dx, dy);
  compute_derivatives(s->ruv, s->druv_dx, s->druv_dy, s->dummy1, s->dummy2, Nx,
                      Ny, dx, dy);
  compute_derivatives(s->rvv, s->dummy1, s->drvv_dy, s->dummy2, s->dummy2, Nx,
                      Ny, dx, dy);
  compute_derivatives(s->eu_p, s->deup_dx, s->dummy1, s->dummy2, s->dummy2, Nx,
                      Ny, dx, dy);
  compute_derivatives(s->ev_p, s->dummy1, s->devp_dy, s->dummy2, s->dummy2, Nx,
                      Ny, dx, dy);
  ...
  for (int i = 1; i < Nx - 1; i++) {
    for (int j = 1; j < Ny - 1; j++) {
      size_t k = IX(i, j);
      rhs_r[k] =
          -(s->dru_dx[k] + s->drv_dy[k]) + k1 * (s->d2r_dx2[k] + s->d2r_dy2[k]);
      rhs_ru[k] =
          -(s->druu_dx[k] + s->druv_dy[k]) - s->dp_dx[k] + k2 * s->d2ru_dx2[k];
      rhs_rv[k] = -(s->druv_dx[k] + s->drvv_dy[k]) - s->dp_dy[k] + c->g * r[k] +
                  k2 * s->d2rv_dy2[k];
      rhs_e[k] = -(s->deup_dx[k] + s->devp_dy[k]) + c->g * r[k] * s->v[k] +
                 k3 * (s->d2e_dx2[k] + s->d2e_dy2[k]);
    }
  }
}

static void apply_bc(const Config *c, double *r, double *ru, double *rv,
                     double *e, double *p) {
  int Nx = c->Nx, Ny = c->Ny;
  for (int j = 0; j < Ny; j++) {
  ...
  for (int i = 0; i < Nx; i++) {
    ...
    e[IX(i, 0)] =
        p[IX(i, 0)] / (c->gamma - 1) +
        0.5 * (ru[IX(i, 0)] * ru[IX(i, 0)] + rv[IX(i, 0)] * rv[IX(i, 0)]) /
            r[IX(i, 0)];
    ...
    e[IX(i, Ny - 1)] = p[IX(i, Ny - 1)] / (c->gamma - 1) +
                       0.5 *
                           (ru[IX(i, Ny - 1)] * ru[IX(i, Ny - 1)] +
                            rv[IX(i, Ny - 1)] * rv[IX(i, Ny - 1)]) /
                           r[IX(i, Ny - 1)];
  }
}
...
static void update_diffusion_coeffs(const Config *c, double dt, double *k1,
                                    double *k2, double *k3) {
  double dx = c->Lx / c->Nx;
  double s = (dx * dx) / (2 * dt);
  *k1 = c->k1_coef * s;
...
static void step(const Config *c, State *s, double dt) {
  ...
  if (strcmp(c->scheme, "euler") == 0) {
    compute_rhs(c, s, s->r, s->ru, s->rv, s->e, s->p, k1, k2, k3, s->rhs_r,
                s->rhs_ru, s->rhs_rv, s->rhs_e);
    for (size_t k = 0; k < n; k++) {
      s->r[k] += dt * s->rhs_r[k];
...
static int prepare_output_dir(const Config *c) {
  if (dir_exists(c->output_dir) && dir_nonempty(c->output_dir)) {
    if (!c->overwrite) {
      fprintf(
          stderr,
          "error: output dir %s exists and is non-empty; "
    ...
    if (system(cmd) != 0) {
      fprintf(stderr, "error: failed to wipe %s\n", c->output_dir);
      return -1;
    }
  }
...
static void dump_config(const Config *c) {
  ...
  fprintf(
      ...
      "  \"scheme\": \"%s\",\n  \"cfl\": %.17g,\n"
      "  \"k1_coef\": %.17g,\n  \"k2_coef\": %.17g,\n  \"k3_coef\": %.17g,\n",
      c->Nx, c->Ny, c->Lx, c->Ly, c->gamma, c->g, c->rho_heavy, c->rho_light,
      c->p0, c->perturbation_amp, c->seed, c->scheme, c->cfl, c->k1_coef,
      c->k2_coef, c->k3_coef);
  if (c->steps < 0)
  ...
  fprintf(fp,
          ...
          "  \"warmup_steps\": %d,\n  \"repeats\": %d\n"
          "}\n",
          c->save_interval, c->output_dir, c->no_output ? "true" : "false",
          c->overwrite ? "true" : "false", c->warmup_steps, c->repeats);
  fclose(fp);
}

static void write_frame(const Config *c, int step_idx, double t,
                        const State *s) {
  char path[1100];
...
int main(int argc, char **argv) {
  ...
  double *wall_times = calloc((size_t)c.repeats, sizeof(double));
  if (!wall_times) {
    fprintf(stderr, "OOM\n");
    return 1;
  }
  ...
  for (int rep = 0; rep < c.repeats; rep++) {
    ...
    for (;;) {
      if (target >= 0 && step_idx >= target)
        break;
      if (!isnan(c.t_end) && t >= c.t_end)
        break;
      double dt = compute_dt(&c, &s);
      if (!isnan(c.t_end) && t + dt > c.t_end)
        dt = c.t_end - t;
  ...
  /* sort wall_times (bubble; repeats is tiny) */
  ...
  double steps_per_s =
      median_wall > 0 ? (double)timed_steps / median_wall : 0.0;
  double cu_per_s =
      median_wall > 0 ? (double)c.Nx * c.Ny * timed_steps / median_wall : 0.0;
  ...
  printf("{\"impl\":\"c\",\"Nx\":%d,\"Ny\":%d,\"scheme\":\"%s\",\"seed\":%d,"
         "\"warmup_steps\":%d,\"repeats\":%d,\"timed_steps\":%ld,"
         "\"wall_s_median\":%.17g,\"wall_s_all\":[",
         c.Nx, c.Ny, c.scheme, c.seed, c.warmup_steps, c.repeats, timed_steps,
         median_wall);
  for (int i = 0; i < c.repeats; i++) {
    printf(i == 0 ? "%.17g" : ",%.17g", wall_times[i]);
  }
  printf("],\"steps_per_s\":%.17g,\"cell_updates_per_s\":%.17g,\"peak_rss_mb\":"
         "%.17g}\n",
         steps_per_s, cu_per_s, peak_rss_mb);
  free(wall_times);
  return 0;
}
